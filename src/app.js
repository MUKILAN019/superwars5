...
class Superwar {
  ...
  calculateScore = () => {
    ...
    // Type your code here
    let calcScore = { hero: 0, villain: 0 };
    this.players.forEach((player) => {
      calcScore[player.type] += player.wins;
    });
    return calcScore;
  };
  ...
  checkWin = () => {
    ...
    // Type your code here
    let totalStrengthHero = this.totalStrength('hero');
    let totalStrengthVilain = this.totalStrength('villain');
    let result;
    if (totalStrengthHero > totalStrengthVilain) {
      result = 'hero';
    } else if (totalStrengthVilain > totalStrengthHero) {
      result = 'villain';
    } else {
      result = 'endure';
    }
